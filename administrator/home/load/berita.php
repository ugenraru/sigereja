...
$total_data = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM `tb_berita` WHERE `judul_berita` LIKE '%$cari%' OR `konten_berita` LIKE '%$cari%'"));
$dataperhalaman = 3;
$halaman = (isset($_GET['halaman'])) ? $_GET['halaman'] : 1;
$start = $dataperhalaman * ($halaman - 1);
$jumlah_hal = ceil($total_data / $dataperhalaman);

$sql = mysqli_query($koneksi, "SELECT * FROM `tb_berita` WHERE `judul_berita` LIKE '%$cari%' OR `konten_berita` LIKE '%$cari%' LIMIT $start, $dataperhalaman");
        ...
        <div class="col-8 align-items-stretch" data-aos="zoom-in" data-aos-delay="100">
          <?php while ($q = mysqli_fetch_assoc($sql)) {
            ...
            <div class="card w-100 shadow-sm mb-4">
              ...
              <div class="card-body">
                <h5 class="card-title"><a href="index.php?page=berita_detail&id=<?= $q['id']; ?>"><?= $q['judul_berita']; ?></a></h5>
                <i class="bi bi-person"></i> <?= $user['nama_pengguna']; ?>&nbsp;&nbsp;-&nbsp;&nbsp;<i class="bi bi-calendar"></i> <?= date('Y-m-d H.i.s'); ?>
                <br />
                <br />
                <p class="card-text"><?= mb_strimwidth($q['konten_berita'], 0, 250, '...'); ?></p>
                <a href="index.php?page=berita_detail&id_berita=<?= $q['id_berita']; ?>" class="btn btn-primary">Baca Selengkapnya</a>
              </div>
            </div>
          <?php } ?>
          <?php $base_url_pagination = 'index.php?page=berita&q=' . $cari; ?>
