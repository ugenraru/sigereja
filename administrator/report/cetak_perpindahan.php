...
<body>
  ...
  <b>Data Perpindahan</b><br><br>
  <table border="1" width="100%">
    <tr>
      <th>No.</th>
      <th>NIK</th>
      <th>Nama</th>
      <th>Lingkungan/Stasi Asal</th>
      <th>KUB Asal</th>
      <th>Lingkungan/Stasi Tujuan</th>
      <th>KUB Tujuan</th>
    </tr>
    <!-- Menampilkan data kub berdasarkan id_kub -->
    <?php
    $no = 1;
    $sql = $koneksi->query("SELECT b.nik ,b.nama_umat, c.nama_lingkungan as nama_lingkungan_asal , d.nama_stasi as nama_stasi_asal, e.nama_lingkungan as nama_lingkungan_tujuan, f.nama_stasi as nama_stasi_tujuan, g.nama_kub as nama_kub_asal ,h.nama_kub as nama_kub_tujuan  from tb_pindah a join tb_umat b on a.id_umat=b.id_umat join tb_lingkungan c on a.id_lingkungan_asal=c.id_lingkungan join tb_stasi d on c.id_stasi=d.id_stasi join tb_lingkungan e on a.id_lingkungan_tujuan=e.id_lingkungan  join tb_stasi f on e.id_stasi=f.id_stasi join tb_kub g on a.id_kub_asal=g.id_kub join tb_kub h on a.id_kub_tujuan=h.id_kub");
    $no = 1;
    while ($data = mysqli_fetch_array($sql, MYSQLI_BOTH)) { ?>
      <tr>
        ...
        <td>
          <?php echo $data['nama_lingkungan_asal'] . ' - ' . $data['nama_stasi_asal']; ?>
        </td>
        <td>
          <?php echo $data['nama_kub_asal']; ?>
        </td>
        <td>
          <?php echo $data['nama_lingkungan_tujuan'] . ' - ' . $data['nama_stasi_tujuan']; ?>
        </td>
        <td>
          <?php echo $data['nama_kub_tujuan']; ?>
        </td>
      </tr>
    <?php }; ?>
  </table><br><br><br>
...
$pdf->WriteHTML($html);
$pdf->Output('Data Perpindahan.pdf','D');
