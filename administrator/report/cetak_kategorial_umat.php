...
{
  // Proses cetak data kategorial dalam bentuk PDF dimana data diambil dari database berdasarkan id_kategorial
  $id_kategorial=$_POST['id_kategorial'];
  $d=mysqli_query($koneksi, "SELECT*FROM tb_kategorial WHERE id_kategorial = '$id_kategorial'");
  $datak=mysqli_fetch_assoc($d);
  $nama_kategorial=$datak['nama_kategorial'];
...
<body>
  ...
  <b>Kelompok Kategorial Umat: <?php echo $nama_kategorial;?></b><br><br>
  <table border="1">
    <tr>
      <th>No.</th>
      <th>Nama Umat</th>
  ...
  <!-- Menampilkan data kategorial berdasarkan id_kategorial -->
<?php $sql_tampil="SELECT * FROM tb_umat, tb_kategorial where tb_umat.id_kategorial=tb_kategorial.id_kategorial 
and tb_umat.id_kategorial='$id_kategorial'";
...
while ($data=mysqli_fetch_array($query_tampil,MYSQLI_BOTH)){?>
    <tr>
      ...
      <td style="text-align:center;"><?php echo $data['alamat'];?> RT <?php echo $data['rt'];?>/ RW <?php echo $data['rw'];?></td>
...
$pdf->WriteHTML($html);
$pdf->Output('Data Umat PerKategorial.pdf', 'D');