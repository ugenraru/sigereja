...
if (isset($_POST['btnLogin'])) {
	...
	//query login mengambil data dari tabel pengguna
	$sql_login = "SELECT * FROM tb_pengguna WHERE BINARY username='$username' AND password='$password'";
	$query_login = mysqli_query($koneksi, $sql_login);
	$data_login = mysqli_fetch_array($query_login, MYSQLI_BOTH);
	$jumlah_login = mysqli_num_rows($query_login);

	//cek login atau cek data pada tabel pengguna
	if ($jumlah_login == 1) {
		session_start();
		...
		//tampilkan peringatan login berhasil dan menuju ke halaman index.php
		echo "<script>
		...
		//tampilkan peringatan login gagal dan menuju ke halaman login.php
		echo "<script>
